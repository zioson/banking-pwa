...
class Auth
{
    ...
    public function resetPassword(int $adminUserId, int $targetUserId): array
    {
        ...
        // ★ FIX (FIN-2b-003/015): Column name corrected to match actual DB schema
        $this->db->query(
            'UPDATE staff SET password_hash = :hash, password_changed_at = NOW(), account_locked = 0,
    ...
    private function createSession(int $userId, string $ip, string $userAgent): string
    {
        ...
        $this->db->query(
            'INSERT INTO sessions (id, staff_id, ip_address, user_agent, created_at, expires_at)
             VALUES (:jti, :staff_id, :ip, :ua, FROM_UNIXTIME(:created), FROM_UNIXTIME(:expires))',
            [
                'jti'     => $tokenId,
                'staff_id' => $userId,
                'ip'       => $ip,
                ...
                'UPDATE staff
                 SET failed_login_attempts = failed_login_attempts + 1,
                     locked_until = CASE
                         ...
                         THEN FROM_UNIXTIME(UNIX_TIMESTAMP() + :lockout_duration)
                         ELSE locked_until
                     END,
                     -- ★ FIX (FIN-2b-003/015): Column name corrected to match actual DB schema
                     account_locked = CASE
    ...
    private function resetFailedAttempts(int $userId): void
    {
        $this->db->query(
            // ★ FIX (FIN-2b-003/015): Column name corrected to match actual DB schema
            'UPDATE staff SET failed_login_attempts = 0, account_locked = 0, locked_until = NULL WHERE id = :id',
            ['id' => $userId]
        );
    }
