...
function generateRef(string $type = 'TXN'): string
{
    ...
    try {
        ...
        // fail on UNIQUE(ref). The caller (transactions.php POST) retries on 23505.
        //
        ...
        $stmt = $db->prepare(
            "SELECT ref FROM transactions 
             WHERE ref LIKE ? 
             ORDER BY CAST(SPLIT_PART(ref, '-', 3) AS INTEGER) DESC 
             LIMIT 1"
        );
        $stmt->execute([$like]);
...
function generateDocumentNumber(string $type = 'STMT'): string
{
    ...
    try {
        ...
        $stmt = $db->prepare(
            "SELECT COALESCE(MAX(CAST(RIGHT(document_number, 4) AS INTEGER)), 0) AS max_seq
...
function generateAccountNumber(): string
{
    ...
    try {
        $db = getDB();
        $stmt = $db->prepare(
            "SELECT MAX(CAST(SUBSTRING(account_number FROM 9) AS INTEGER)) AS max_seq
             FROM accounts
...
function generateCustomerNumber(): string
{
    try {
        $db = getDB();
        $stmt = $db->prepare(
            "SELECT MAX(CAST(SUBSTRING(customer_number FROM 6) AS INTEGER)) AS max_seq
             FROM customers
    ...
    try {
        ...
        $journalStmt = $db->prepare(
            "INSERT INTO gl_journal (journal_ref, transaction_type, branch, description, total_debit, total_credit, is_balanced, posted_by)
             ...
             ON CONFLICT (journal_ref) DO UPDATE SET
...
function createSession(int $staffId, string $ipAddress = '', string $userAgent = ''): string
{
    ...
    try {
        ...
        if ($activeSessions >= $maxConcurrent) {
            ...
            // PostgreSQL: DELETE with subquery instead of ORDER BY/LIMIT (not supported in DELETE)
            $deleteStmt = $db->prepare(
                'DELETE FROM sessions WHERE id IN (
                    SELECT id FROM sessions WHERE staff_id = :staff_id AND expires_at > NOW()
...
function _ensureStaffColumns(PDO $db): void
{
    ...
    try {
        ...
        if (!isset($cols['last_login_ip'])) {
            $db->exec('ALTER TABLE staff ADD COLUMN last_login_ip VARCHAR(50) DEFAULT NULL');
        }
...
function _ensureSessionColumns(PDO $db): void
{
    ...
    try {
        try {
            $tbl = $db->query("SELECT table_name FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'sessions'")->fetchColumn();
            if (!$tbl) {
                ...
                $db->exec("CREATE INDEX IF NOT EXISTS idx_staff ON sessions (staff_id)");
                $db->exec("CREATE INDEX IF NOT EXISTS idx_expires ON sessions (expires_at)");
                $db->exec("CREATE INDEX IF NOT EXISTS idx_last_activity ON sessions (last_activity)");
            }
        } catch (PDOException $e) {}
        ...
        if (isset($cols['id']) && is_string($cols['id'])) {
            ...
            $len = 0;
            if (preg_match('/(character varying|varchar|char)\((\d+)\)/i', $t, $m)) {
                $len = (int)$m[2];
            }
            if ($len > 0 && $len < 64) {
                try { $db->exec('ALTER TABLE sessions ALTER COLUMN id TYPE VARCHAR(128)'); } catch (PDOException $e) {}
            }
        }

        if (!isset($cols['last_activity'])) {
            $db->exec('ALTER TABLE sessions ADD COLUMN last_activity TIMESTAMP DEFAULT CURRENT_TIMESTAMP');
        }
        if (!isset($cols['device_fingerprint'])) {
            $db->exec("ALTER TABLE sessions ADD COLUMN device_fingerprint VARCHAR(64) DEFAULT ''");
        }
        if (!isset($cols['label'])) {
            $db->exec("ALTER TABLE sessions ADD COLUMN label VARCHAR(255) DEFAULT ''");
        }
...
function _ensureLoginHistoryColumns(PDO $db): void
{
    ...
    try {
        ...
        if (!isset($cols['device_fingerprint'])) {
            $db->exec("ALTER TABLE login_history ADD COLUMN device_fingerprint VARCHAR(64) DEFAULT ''");
        }
...
function getSetting(PDO $db, string $key, $fallback = null) {
    try {
        ...
        // ★ PG-MIGRATION FIX: Use value_data directly (the actual column name in PG schema).
        // Previous try/fallback approach threw a PDOException on every call (wasteful + noisy logs).
