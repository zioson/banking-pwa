...
function generateRef(string $type = 'TXN'): string
{
    ...
    try {
        ...
        //
        // FIXED: Removed 'created_at >= CURDATE()' filter.
        ...
        $stmt = $db->prepare(
            "SELECT ref FROM transactions 
             WHERE ref LIKE ? 
             ORDER BY CAST(SUBSTRING_INDEX(ref, '-', -1) AS UNSIGNED) DESC 
             LIMIT 1"
        );
        $stmt->execute([$like]);
...
function generateDocumentNumber(string $type = 'STMT'): string
{
    ...
    try {
        $db = getDB();
        // ★ FIX (FIN-2b-010): Removed COUNT(*) + CURDATE() approach (timezone bug:
        // MySQL CURDATE() vs PHP date() timezone mismatch caused sequence resets).
        ...
        $stmt = $db->prepare(
            "SELECT COALESCE(MAX(CAST(SUBSTRING(document_number, -4) AS UNSIGNED)), 0) AS max_seq
...
function generateAccountNumber(): string
{
    ...
    try {
        $db = getDB();
        $stmt = $db->prepare(
            "SELECT MAX(CAST(SUBSTRING(account_number, 9) AS UNSIGNED)) AS max_seq
             FROM accounts
...
function generateCustomerNumber(): string
{
    try {
        $db = getDB();
        $stmt = $db->prepare(
            "SELECT MAX(CAST(SUBSTRING(customer_number, 6) AS UNSIGNED)) AS max_seq
             FROM customers
...
function _ensureAuditLogColumns(): void
{
    ...
    try {
        $db = getDB();
        $cols = [];
        foreach ($db->query('SHOW COLUMNS FROM audit_logs')->fetchAll(PDO::FETCH_ASSOC) as $c) {
            $cols[strtolower($c['Field'])] = true;
        }

        if (!isset($cols['module'])) {
            $db->exec("ALTER TABLE audit_logs ADD COLUMN `module` VARCHAR(100) DEFAULT '' AFTER `details`");
        }
        if (!isset($cols['category'])) {
            $db->exec("ALTER TABLE audit_logs ADD COLUMN `category` VARCHAR(100) DEFAULT '' AFTER `module`");
        }
        if (!isset($cols['user_agent'])) {
            $db->exec("ALTER TABLE audit_logs ADD COLUMN `user_agent` VARCHAR(500) DEFAULT '' AFTER `category`");
        }
...
function postToGL(
    ...
    try {
        $db = getDB();
        // Minimal self-heal so GL posting never fails due to missing table on older installs.
        $db->exec("CREATE TABLE IF NOT EXISTS general_ledger (
            ...
            INDEX idx_gl_ref (reference),
            INDEX idx_gl_code (account_code),
            INDEX idx_gl_branch (branch)
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS gl_journal (
            ...
            posted_by INT DEFAULT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE KEY uk_gl_journal_ref (journal_ref)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        ...
        $stmt = $db->prepare(
            "INSERT INTO general_ledger (account_code, account_name, debit, credit, date, reference, description, posted_by, transaction_type, contra_account, branch)
             VALUES (:code, :name, :debit, :credit, CURDATE(), :ref, :desc, :by, :type, :contra, :branch)"
        );
...
function processTransaction(string $template, array $payload): void
{
    ...
    switch ($tpl) {
        ...
        case 'LOAN_PRINCIPAL_REPAYMENT':
            $drCode = '2000'; $crCode = '1201'; $txnType = 'LOAN_PRINCIPAL_REPAYMENT';
            $defaultDesc = 'Loan principal repayment';
            break;
        case 'LOAN_INTEREST_PAYMENT':
            $drCode = '2000'; $crCode = '4200'; $txnType = 'LOAN_INTEREST_PAYMENT';
...
function _ensureStaffColumns(PDO $db): void
{
    ...
    try {
        $cols = [];
        foreach ($db->query('SHOW COLUMNS FROM staff')->fetchAll(PDO::FETCH_ASSOC) as $c) {
            $cols[strtolower($c['Field'])] = true;
        }

        if (!isset($cols['force_password_change'])) {
            $db->exec("ALTER TABLE staff ADD COLUMN force_password_change TINYINT(1) NOT NULL DEFAULT 0 AFTER mfa_required");
        }
        ...
        if (!isset($cols['last_login_ip'])) {
            $db->exec("ALTER TABLE staff ADD COLUMN last_login_ip VARCHAR(50) DEFAULT NULL AFTER last_login");
        }
...
function _ensureSessionColumns(PDO $db): void
{
    ...
    try {
        try {
            $tbl = $db->query("SHOW TABLES LIKE 'sessions'")->fetchColumn();
            if (!$tbl) {
        ...
        $cols = [];
        $rawCols = $db->query('SHOW COLUMNS FROM sessions')->fetchAll(PDO::FETCH_ASSOC);
        ...
        if (isset($cols['id']) && is_string($cols['id'])) {
            ...
            if ($len > 0 && $len < 64) {
                try { $db->exec("ALTER TABLE sessions MODIFY COLUMN id VARCHAR(128) NOT NULL"); } catch (PDOException $e) {}
            }
        }
        ...
        if (!isset($cols['device_fingerprint'])) {
            $db->exec("ALTER TABLE sessions ADD COLUMN device_fingerprint VARCHAR(64) DEFAULT '' AFTER last_activity");
        }
        if (!isset($cols['label'])) {
            $db->exec("ALTER TABLE sessions ADD COLUMN label VARCHAR(255) DEFAULT '' AFTER device_fingerprint");
        }
...
function _ensureLoginHistoryColumns(PDO $db): void
{
    ...
    try {
        $cols = [];
        foreach ($db->query('SHOW COLUMNS FROM login_history')->fetchAll(PDO::FETCH_ASSOC) as $c) {
            $cols[strtolower($c['Field'])] = true;
        }

        if (!isset($cols['device_fingerprint'])) {
            $db->exec("ALTER TABLE login_history ADD COLUMN device_fingerprint VARCHAR(64) DEFAULT '' AFTER risk");
        }
