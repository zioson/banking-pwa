...
class AuditLogger
{
    ...
    public function getLogs(array $filters = [], int $page = 1, int $pageSize = 50): array
    {
        ...
        $data = $this->db->fetchAll(
            "SELECT * FROM audit_logs WHERE {$whereClause} ORDER BY created_at DESC LIMIT CAST(:limit AS INTEGER) OFFSET CAST(:offset AS INTEGER)",
    ...
    public function getEntityHistory(string $entity, string|int $entityId, int $limit = 50): array
    {
        return $this->db->fetchAll(
            'SELECT * FROM audit_logs WHERE entity = :entity AND entity_id = :entity_id ORDER BY created_at DESC LIMIT CAST(:limit AS INTEGER)',
    ...
    {
        return $this->db->fetchAll(
            'SELECT * FROM audit_logs WHERE actor = :actor ORDER BY created_at DESC LIMIT CAST(:limit AS INTEGER)',
