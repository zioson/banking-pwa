...
switch ($method) {
    ...
    case 'PUT':
        ...
        try {
            ...
            $glCols = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'branch'")->fetchAll();
            if (empty($glCols)) {
                $db->exec("ALTER TABLE general_ledger ADD COLUMN branch VARCHAR(100) DEFAULT '' AFTER description");
            ...
            $ttCols = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'transaction_type'")->fetchAll();
            if (empty($ttCols)) {
            ...
            $caCols = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'contra_account'")->fetchAll();
            if (empty($caCols)) {
