...
function invResolveBranchContext(PDO $db, array $staff, string $requestedBranch = ''): array
{
 ...
 $rows = [];
 $hasBranchesTable = $db->query("SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'branches'")->fetch();
...
{
 $r = $db->query("SHOW COLUMNS FROM "$table" LIKE " . $db->quote($col))->fetch();
 if (!$r) {
...
function invEnsureSchema(PDO $db): void
{
 ...
 dividend_paid SMALLINT NOT NULL DEFAULT 0,
 actual_div_rate DECIMAL(10,4);
 ...
 invAddCol($db, 'investment_cycles', 'branch', "VARCHAR(100);
 ...
 VALUES ('1450', 'Investment Reserve Fund', 'ASSET', 'Current Assets', 'Bank reserve for investment cycle commitments', 1)")
 ->execute();
 } catch (Throwable $_) {}
}
...
function invLoadState(PDO $db, array $staff, string $requestedBranch = ''): array
{
 ...
 WHERE c.dividend_paid = 1 AND c.branch = :br
 GROUP BY c.id
...
if ($method === 'POST' && $id === 'cycles' && $sub === '') {
 ...
 validationError(['endDate' => 'End date must be after start date.'], 'Validation failed');
 }
...
if ($method === 'POST' && $id === 'dividends') {
 ...
 errorResponse('Dividends can only be paid after a full one-year cycle has completed.', 422);
 }
 ...
 $db->prepare("UPDATE investment_cycles SET dividend_paid = 1, actual_div_rate = :r, status = 'completed' WHERE id = :id")
