...
$db->exec("CREATE TABLE IF NOT EXISTS accounts (
    ...
    status          VARCHAR(30)  NOT NULL DEFAULT 'PENDING_OPENING',
    ...
    tax_exemptions  TEXT         DEFAULT NULL,
...
try {
    $db->exec("CREATE INDEX IF NOT EXISTS idx_accounts_branch ON accounts (branch)");
} catch (PDOException $e) {}
try {
    $db->exec("CREATE INDEX IF NOT EXISTS idx_accounts_status ON accounts (status)");
} catch (PDOException $e) {}
try {
    $db->exec("CREATE INDEX IF NOT EXISTS idx_accounts_customer ON accounts (customer_id)");
} catch (PDOException $e) {}
// ★ FIX (CUST-002): Add hold_balance column if it doesn't exist (migration for existing DBs)
try {
    $db->exec("ALTER TABLE accounts ADD COLUMN hold_balance DECIMAL(20,2) NOT NULL DEFAULT 0");
