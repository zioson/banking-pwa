...
function ensureDocumentColumns(PDO $db): void {
    ...
    try {
        $cols = [];
        foreach ($db->query('SHOW COLUMNS FROM generated_documents')->fetchAll(PDO::FETCH_ASSOC) as $c) {
            $cols[strtolower($c['Field'])] = true;
        }
        // generated_at → created_at
        if (isset($cols['generated_at']) && !isset($cols['created_at'])) {
            $db->exec("ALTER TABLE generated_documents CHANGE `generated_at` `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
            $cols['created_at'] = true;
        }
        // period_from → period_start
        if (isset($cols['period_from']) && !isset($cols['period_start'])) {
            $db->exec("ALTER TABLE generated_documents CHANGE `period_from` `period_start` VARCHAR(50) DEFAULT NULL");
            $cols['period_start'] = true;
        }
        // period_to → period_end
        if (isset($cols['period_to']) && !isset($cols['period_end'])) {
            $db->exec("ALTER TABLE generated_documents CHANGE `period_to` `period_end` VARCHAR(50) DEFAULT NULL");
    ...
    addColumnIfMissing($db, 'generated_documents', 'subtype', "VARCHAR(100) DEFAULT NULL");
    addColumnIfMissing($db, 'generated_documents', 'summary', "JSON DEFAULT NULL");
    addColumnIfMissing($db, 'generated_documents', 'branch', "VARCHAR(100) DEFAULT NULL");
    addColumnIfMissing($db, 'generated_documents', 'account_type', "VARCHAR(50) DEFAULT NULL");
    addColumnIfMissing($db, 'generated_documents', 'customer_id', "VARCHAR(50) DEFAULT NULL");
    addColumnIfMissing($db, 'generated_documents', 'generated_by_name', "VARCHAR(255) DEFAULT NULL");

    // ── Fix restrictive ENUMs to VARCHAR ──
    try {
        $col = $db->query("SHOW COLUMNS FROM generated_documents LIKE 'type'")->fetch();
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM generated_documents LIKE 'status'")->fetch();
        if ($col && str_contains($col['Type'], 'enum(')) {
            $db->exec("ALTER TABLE generated_documents MODIFY COLUMN `status` VARCHAR(20) DEFAULT 'FINAL'");
        }
    } catch (PDOException $e) {
