...
function ensureGeneralLedgerTable(PDO $db): void {
    ...
    $col = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'transaction_type'")->fetch();
    if (!$col) {
        $db->exec("ALTER TABLE general_ledger ADD COLUMN transaction_type VARCHAR(50) DEFAULT '' AFTER posted_by");
    ...
    $col2 = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'contra_account'")->fetch();
    if (!$col2) {
        $db->exec("ALTER TABLE general_ledger ADD COLUMN contra_account VARCHAR(50) DEFAULT '' AFTER transaction_type");
    } else {
        ...
        $colType = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'contra_account'")->fetch();
        if ($colType && strpos($colType['Type'], 'VARCHAR(10)') !== false) {
            $db->exec("ALTER TABLE general_ledger MODIFY COLUMN contra_account VARCHAR(50) DEFAULT ''");
        }
    }
    // ★ Safe migration: add branch column if missing (for branch-level GL filtering)
    $col3 = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'branch'")->fetch();
    if (!$col3) {
        $db->exec("ALTER TABLE general_ledger ADD COLUMN branch VARCHAR(100) DEFAULT '' AFTER contra_account");
        $brIdx = $db->query("SHOW INDEX FROM general_ledger WHERE Key_name = 'idx_branch'")->fetch();
    ...
    try {
        ...
        if ((int)$orphans['c'] > 0) {
            $backfilled = $db->exec(
                "UPDATE general_ledger gl INNER JOIN staff s ON gl.posted_by = s.id SET gl.branch = COALESCE(NULLIF(s.department,''), '') WHERE (gl.branch IS NULL OR gl.branch = '') AND gl.posted_by IS NOT NULL"
            );
            if ($backfilled > 0) {
    ...
    foreach ($criticalCodes as $row) {
        ...
        if ($existing) {
            if ($existing['name'] !== $row[1]) {
                $db->prepare("UPDATE chart_of_accounts SET name = ?, type = ?, category = ?, description = ?, is_active = 1 WHERE code = ?")
            ...
            if ($trialDifference >= 0.01) {
                $diagStmt = $db->prepare(
                    "SELECT reference,
                        ...
                        GROUP_CONCAT(DISTINCT transaction_type) AS tx_types,
                        GROUP_CONCAT(DISTINCT account_code ORDER BY account_code) AS accounts,
    ...
    case 'POST':
        ...
        try {
            ...
            if ($action === 'FUND_TRANSFER') {
                ...
                $glAcc = $db->prepare("SELECT * FROM chart_of_accounts WHERE code = ? AND is_active = 1");
                ...
                $ftCol1 = $db->query("SHOW COLUMNS FROM operating_account_transactions LIKE 'contra_account'")->fetch();
                if (!$ftCol1) { $db->exec("ALTER TABLE operating_account_transactions ADD COLUMN contra_account VARCHAR(100) DEFAULT '' AFTER operator"); }
                $ftCol2 = $db->query("SHOW COLUMNS FROM operating_account_transactions LIKE 'transaction_type'")->fetch();
                if (!$ftCol2) { $db->exec("ALTER TABLE operating_account_transactions ADD COLUMN transaction_type VARCHAR(50) DEFAULT '' AFTER contra_account"); }

                // ★ Safe migration: add branch column to operating_account_transactions
                $ftCol3 = $db->query("SHOW COLUMNS FROM operating_account_transactions LIKE 'branch'")->fetch();
                if (!$ftCol3) { $db->exec("ALTER TABLE operating_account_transactions ADD COLUMN branch VARCHAR(100) DEFAULT '' AFTER transaction_type"); }
                ...
                try {
                    ...
                    $db->prepare("INSERT INTO operating_account_transactions (ref, operating_account_id, date, type, description, amount, balance_after, operator, contra_account, transaction_type, branch) VALUES (?, ?, CURDATE(), ?, ?, ?, ?, ?, ?, ?, ?)")
                    ...
                    $db->prepare("INSERT INTO general_ledger (account_code, account_name, debit, credit, date, reference, description, branch, posted_by, transaction_type, contra_account) VALUES (?, ?, ?, ?, CURDATE(), ?, ?, ?, ?, ?, ?)")
            ...
            if ($action === 'LOAN_FUND_CREDIT') {
                ...
                $srcStmt = $db->prepare("SELECT * FROM chart_of_accounts WHERE code = ? AND is_active = 1");
                ...
                $db->exec("CREATE TABLE IF NOT EXISTS loan_fund_accounts (
                    ...
                    fund_type ENUM('LOAN_FUND','LOAN_INTEREST') NOT NULL,
                    balance DECIMAL(20,2) DEFAULT 0,
                    currency VARCHAR(5) DEFAULT 'XAF',
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
                ...
                try {
                    ...
                    $db->prepare("INSERT INTO loan_fund_transactions (ref, loan_fund_account_id, loan_id, transaction_ref, date, type, description, amount, balance_after, operator) VALUES (?, ?, NULL, ?, CURDATE(), 'CREDIT', ?, ?, ?)")
            ...
            if ($action === 'LOAN_FUND_DEBIT') {
                ...
                $db->exec("CREATE TABLE IF NOT EXISTS loan_fund_accounts (
                    id INT AUTO_INCREMENT PRIMARY KEY, account_number VARCHAR(30) NOT NULL UNIQUE,
                    account_name VARCHAR(200) NOT NULL, fund_type ENUM('LOAN_FUND','LOAN_INTEREST') NOT NULL,
                    balance DECIMAL(20,2) DEFAULT 0, currency VARCHAR(5) DEFAULT 'XAF',
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
                $db->exec("CREATE TABLE IF NOT EXISTS loan_fund_transactions (
                    id INT AUTO_INCREMENT PRIMARY KEY, ref VARCHAR(50), loan_fund_account_id INT NOT NULL,
                    loan_id INT DEFAULT NULL, transaction_ref VARCHAR(50) DEFAULT NULL, date DATE NOT NULL,
                    type ENUM('CREDIT','DEBIT') NOT NULL, description TEXT, amount DECIMAL(20,2) NOT NULL,
                ...
                try {
                    ...
                    if ($lfId > 0) {
                        ...
                        $db->prepare("INSERT INTO loan_fund_transactions (ref, loan_fund_account_id, loan_id, transaction_ref, date, type, description, amount, balance_after, operator) VALUES (?, ?, ?, ?, CURDATE(), 'DEBIT', ?, ?, ?)")
            ...
            if ($action === 'LOAN_REPAYMENT_PRINCIPAL') {
                ...
                try {
                    ...
                    if ($lfId > 0) {
                        $lfCount = (int)$db->query("SELECT COUNT(*) FROM loan_fund_transactions WHERE loan_fund_account_id = " . $lfId)->fetchColumn();
                        $lfRef = 'LF-' . str_replace('-', '', date('Y-m-d')) . '-' . str_pad($lfCount + 1, 3, '0', STR_PAD_LEFT);
                        $db->prepare("INSERT INTO loan_fund_transactions (ref, loan_fund_account_id, loan_id, transaction_ref, date, type, description, amount, balance_after, operator) VALUES (?, ?, ?, ?, CURDATE(), 'CREDIT', ?, ?, ?)")
            ...
            $dStmt = $db->prepare("SELECT * FROM chart_of_accounts WHERE code = ? AND is_active = 1");
            ...
            $cStmt = $db->prepare("SELECT * FROM chart_of_accounts WHERE code = ? AND is_active = 1");
            ...
            try {
                ...
                if ($debitAccount === '1200') {
                    ...
                    if ($lfAutoRow) {
                        ...
                        // ★ BUG FIX: Query GL 1200 balance AFTER the debit insert.
