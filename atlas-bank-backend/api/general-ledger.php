...
function ensureGeneralLedgerTable(PDO $db): void {
 ...
 if (!$col2) {
 $db->exec("ALTER TABLE general_ledger ADD COLUMN contra_account VARCHAR(50);
 } else {
 ...
 if (!$col3) {
 $db->exec("ALTER TABLE general_ledger ADD COLUMN branch VARCHAR(100);
 $brIdx = $db->query("SELECT indexname FROM pg_indexes WHERE tablename = 'general_ledger' AND indexname = 'idx_branch'")->fetch();
 if (!$brIdx) { $db->exec("ALTER TABLE general_ledger ADD"); }
 }
...
switch ($method) {
 ...
 STRING_AGG(DISTINCT transaction_type::text, ',') AS tx_types,
 STRING_AGG(DISTINCT account_code ORDER BY account_code::text, ',') AS accounts,
 ...
 if (!$ftCol1) { $db->exec("ALTER TABLE operating_account_transactions ADD COLUMN contra_account VARCHAR(100); }
 ...
 if (!$ftCol2) { $db->exec("ALTER TABLE operating_account_transactions ADD COLUMN transaction_type VARCHAR(50); }
 ...
 if (!$ftCol3) { $db->exec("ALTER TABLE operating_account_transactions ADD COLUMN branch VARCHAR(100); }
 ...
 account_name VARCHAR(200) NOT NULL, fund_type VARCHAR(30) NOT NULL DEFAULT 'LOAN_FUND',
 balance DECIMAL(20,2);
 $db->exec("CREATE TABLE IF NOT EXISTS loan_fund_transactions (
 id SERIAL PRIMARY KEY, ref VARCHAR(50), loan_fund_account_id INT NOT NULL,
 loan_id INT DEFAULT NULL, transaction_ref VARCHAR(50);
