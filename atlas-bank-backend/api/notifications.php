...
try {
    ...
    $colCheck = $db->query("SHOW COLUMNS FROM `notifications` LIKE 'is_read'")->fetch();
    if (!$colCheck) {
        $db->exec("ALTER TABLE `notifications` ADD COLUMN `is_read` TINYINT(1) DEFAULT 0 AFTER `status`");
    }
    $colCheck2 = $db->query("SHOW COLUMNS FROM `notifications` LIKE 'created_at'")->fetch();
    if (!$colCheck2) {
...
switch ($method) {
    ...
    case 'GET':
        ...
        try {
            ...
            $stmt = $db->prepare('SELECT * FROM notifications ' . $where . ' ORDER BY timestamp DESC LIMIT :limit OFFSET :offset');
    ...
    case 'PUT':
        ...
        if ($id === 'mark-all-read') {
            try {
                ...
                $tableExists = $db->query("SELECT 1 FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'notifications' LIMIT 1")->fetch();
                if (!$tableExists) {
                    serverErrorResponse('Notifications table does not exist. Please reload the page to trigger schema creation.');
                    break;
                }

                $statusCol = $db->query("SHOW COLUMNS FROM `notifications` LIKE 'status'")->fetch();
                $staffCol  = $db->query("SHOW COLUMNS FROM `notifications` LIKE 'target_staff_id'")->fetch();
