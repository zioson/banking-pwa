...
function opEnsureTransactionsTable(PDO $db): void {
 ...
 opAddCol($db, 'operating_account_transactions', 'contra_account', "VARCHAR(100);
 // Safe migration: add transaction_type column if missing
 opAddCol($db, 'operating_account_transactions', 'transaction_type', "VARCHAR(50);
 ...
 opAddCol($db, 'operating_account_transactions', 'branch', "VARCHAR(100);
 ...
 if (!$brIdx) {
 $db->exec("ALTER TABLE operating_account_transactions ADD");
 }
 ...
 if (!$col2) {
 $db->exec("ALTER TABLE "operating_account_transactions" RENAME COLUMN "timestamp" TO "created_at" DEFAULT CURRENT_TIMESTAMP");
 }
 }
}
...
function opEnsureGeneralLedgerTable(PDO $db): void {
 ...
 if (!$glCol) {
 $db->exec("ALTER TABLE general_ledger ADD COLUMN transaction_type VARCHAR(50);
 $db->exec("ALTER TABLE general_ledger ADD");
 }
 // Safe migration: add contra_account column
 opAddCol($db, 'general_ledger', 'contra_account', "VARCHAR(50);
 // Safe migration: add branch column for branch isolation
 opAddCol($db, 'general_ledger', 'branch', "VARCHAR(100);
...
function opEnsureChartOfAccounts(PDO $db): void {
 ...
 $ins = $db->prepare("INSERT INTO chart_of_accounts (code, name, type, category, description, is_active) VALUES (?, ?, ?, ?, ?, 1)");
...
switch ($method) {
 ...
 // ── Success response ─────────────────────────────────────────
 successResponse([
 'id' => (int)$db->lastInsertId('general_ledger_id_seq'),
 'ref' => $ref,
 'type' => $type,
 'amount' => $amount,
