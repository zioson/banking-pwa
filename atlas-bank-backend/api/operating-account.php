 ...
 *     after successful commit, but is never the authoritative source
...
function opEnsureTransactionsTable(PDO $db): void {
    ...
    opAddCol($db, 'operating_account_transactions', 'contra_account', "VARCHAR(100) DEFAULT '' AFTER operator");
    // Safe migration: add transaction_type column if missing
    opAddCol($db, 'operating_account_transactions', 'transaction_type', "VARCHAR(50) DEFAULT '' AFTER contra_account");
    ...
    opAddCol($db, 'operating_account_transactions', 'branch', "VARCHAR(100) DEFAULT '' AFTER transaction_type");
    ...
    $col = $db->query("SHOW COLUMNS FROM operating_account_transactions LIKE 'timestamp'")->fetch();
    if ($col) {
        $col2 = $db->query("SHOW COLUMNS FROM operating_account_transactions LIKE 'created_at'")->fetch();
        if (!$col2) {
...
function opEnsureGeneralLedgerTable(PDO $db): void {
    ...
    $glCol = $db->query("SHOW COLUMNS FROM general_ledger LIKE 'transaction_type'")->fetch();
    if (!$glCol) {
    ...
    opAddCol($db, 'general_ledger', 'contra_account', "VARCHAR(50) DEFAULT '' AFTER transaction_type");
    // Safe migration: add branch column for branch isolation
    opAddCol($db, 'general_ledger', 'branch', "VARCHAR(100) DEFAULT '' AFTER contra_account");
...
function opEnsureChartOfAccounts(PDO $db): void {
    ...
    $ins = $db->prepare("INSERT IGNORE INTO chart_of_accounts (code, name, type, category, description, is_active) VALUES (?, ?, ?, ?, ?, 1)");
...
function opGenerateRef(PDO $db): string {
    ...
    // ★ FIX (OPFUND-B002): Removed COUNT(*) + CURDATE() approach.
...
switch ($method) {
    ...
    case 'GET':
        try {
            ...
            foreach ($records as &$record) {
                try {
                    $tSql = 'SELECT * FROM operating_account_transactions WHERE operating_account_id = :aid' . $txnBranchFilter . ' ORDER BY created_at DESC LIMIT :lim';
                    ...
                    try {
                        $tSql2 = 'SELECT * FROM operating_account_transactions WHERE operating_account_id = :aid' . $txnBranchFilter . ' ORDER BY id DESC LIMIT :lim';
