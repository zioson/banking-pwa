...
try {
 $_alCols = [];
 foreach ($db->query("SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'audit_logs'")->fetchAll(PDO::FETCH_ASSOC) as $_c) {
 ...
 $db->exec("ALTER TABLE audit_logs ADD");
 }
 // Enterprise columns: module, category, user_agent
 if (!isset($_alCols['module'])) {
 $db->exec("ALTER TABLE audit_logs ADD COLUMN module VARCHAR(50);
 $db->exec("ALTER TABLE audit_logs ADD");
 }
 if (!isset($_alCols['category'])) {
 $db->exec("ALTER TABLE audit_logs ADD COLUMN category VARCHAR(50);
 $db->exec("ALTER TABLE audit_logs ADD");
 }
...
function buildAuditDashboard(array $staff): array {
 ...
 STRING_AGG(DISTINCT t.account::text, ',') AS accounts,
 STRING_AGG(DISTINCT t.type::text, ',') AS tx_types,
