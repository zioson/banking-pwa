...
try {
    $_alCols = [];
    foreach ($db->query("SHOW COLUMNS FROM audit_logs")->fetchAll(PDO::FETCH_ASSOC) as $_c) {
        $_alCols[strtolower($_c['Field'])] = true;
    }
    // Fix: created_at → timestamp
    if (!isset($_alCols['timestamp'])) {
        if (isset($_alCols['created_at'])) {
            $db->exec("ALTER TABLE audit_logs CHANGE COLUMN created_at timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
        } else {
            $db->exec("ALTER TABLE audit_logs ADD COLUMN timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP AFTER details");
        }
        $db->exec("ALTER TABLE audit_logs ADD INDEX idx_timestamp (timestamp)");
    }
    // Enterprise columns: module, category, user_agent
    if (!isset($_alCols['module'])) {
        $db->exec("ALTER TABLE audit_logs ADD COLUMN module VARCHAR(50) DEFAULT '' AFTER details");
        $db->exec("ALTER TABLE audit_logs ADD INDEX idx_module (module)");
    }
    if (!isset($_alCols['category'])) {
        $db->exec("ALTER TABLE audit_logs ADD COLUMN category VARCHAR(50) DEFAULT '' AFTER module");
        $db->exec("ALTER TABLE audit_logs ADD INDEX idx_category (category)");
    }
...
switch ($method) {
    ...
    case 'GET':
        ...
        if ($id === 'findings') {
            ...
            try {
                ...
                $stmt = $db->prepare('SELECT * FROM audit_findings ' . $where . ' ORDER BY timestamp DESC LIMIT :limit OFFSET :offset');
        ...
        try {
            ...
            $countStmt->execute();
            $total = (int)$countStmt->fetchAll(PDO::FETCH_ASSOC)[0]['total'] ?? 0;
            $stmt = $db->prepare('SELECT * FROM audit_logs ' . $where . ' ORDER BY timestamp DESC LIMIT :limit OFFSET :offset');
            foreach ($params as $k => $v) { $stmt->bindValue($k, $v); }
            $stmt->bindValue(':limit', $pageSize, PDO::PARAM_INT);
            $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
            $stmt->execute();
    ...
    case 'POST':
        ...
        if ($id === 'findings') {
            ...
            try {
                $db = getDB();
                $stmt = $db->prepare('INSERT INTO audit_findings (severity, category, description, recommendation, branch, status, assignee, created_date, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, CURDATE(), ?)');
...
function buildAuditDashboard(array $staff): array {
    ...
    $unbStmt = $db->prepare("
        SELECT t.ref AS reference,
               ...
               GROUP_CONCAT(DISTINCT t.account) AS accounts,
               GROUP_CONCAT(DISTINCT t.type) AS tx_types,
