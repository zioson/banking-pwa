...
function loanAddCol(PDO $db, string $table, string $col, string $def): void {
    try {
        $r = $db->query("SHOW COLUMNS FROM `$table` LIKE '$col'")->fetch();
...
function loanRepairCorruptedNumbers(PDO $db): void {
    ...
    try {
        $corruptedRows = $db->query(
            "SELECT id, loan_number FROM loans WHERE loan_number REGEXP 'E\\+|e\\+' AND loan_number LIKE 'LN-%'"
        ...
        $yearSeq = [];
        $validRows = $db->query(
            "SELECT loan_number FROM loans WHERE loan_number REGEXP '^LN-[0-9]{4}-[0-9]{3}$'"
...
function loanEnsureTransactionsCompat(PDO $db): void {
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS transactions (
            ...
            PRIMARY KEY (id),
            UNIQUE KEY uk_txn_ref (ref),
            INDEX idx_txn_branch (branch),
            INDEX idx_txn_status (status),
            INDEX idx_txn_account (account),
...
function loanInsertTransaction(PDO $db, array $row): void {
    $cols = [];
    try {
        $colRows = $db->query("SHOW COLUMNS FROM transactions")->fetchAll(PDO::FETCH_COLUMN, 0);
...
function loanEnsureSchema(PDO $db): void {
    ...
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS loans (
            ...
            status ENUM('PENDING','UNDER_REVIEW','APPROVED','ACTIVE','DELINQUENT','CLOSED','DEFAULTED','REJECTED','RESTRUCTURED','WRITTEN_OFF','SETTLED') NOT NULL DEFAULT 'PENDING',
            ...
            term_months INT UNSIGNED NOT NULL DEFAULT 0,
            repayment_freq VARCHAR(30) DEFAULT 'Monthly',
            disbursed_at DATE DEFAULT NULL,
            maturity_date DATE DEFAULT NULL,
            next_due DATE DEFAULT NULL,
            debit_account_id INT UNSIGNED DEFAULT NULL,
            debit_account_number VARCHAR(30) DEFAULT NULL,
            guarantor_customer_id INT UNSIGNED DEFAULT NULL,
            guarantor_account_id INT UNSIGNED DEFAULT NULL,
    ...
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS loan_schedule (
            ...
            status ENUM('DUE','PAID','MISSED','PARTIALLY_PAID') NOT NULL DEFAULT 'DUE',
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM loan_schedule LIKE 'due'")->fetch();
        if ($col) {
            $col2 = $db->query("SHOW COLUMNS FROM loan_schedule LIKE 'due_date'")->fetch();
            if (!$col2) {
                $db->exec("ALTER TABLE loan_schedule CHANGE `due` `due_date` DATE NOT NULL");
            }
        }
    } catch (PDOException $e) {
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM loans LIKE 'repayment_mode'")->fetch();
        if ($col && str_contains($col['Type'], 'enum(')) {
            $db->exec("ALTER TABLE loans MODIFY COLUMN repayment_mode VARCHAR(30) NOT NULL DEFAULT 'SCHEDULED'");
        }
    } catch (PDOException $e) {
    ...
    try {
        $col2 = $db->query("SHOW COLUMNS FROM loans LIKE 'status'")->fetch();
        if ($col2 && str_contains($col2['Type'], 'enum(')) {
            if (!str_contains($col2['Type'], "'UNDER_REVIEW'")) {
                $db->exec("ALTER TABLE loans MODIFY COLUMN status ENUM('PENDING','UNDER_REVIEW','APPROVED','ACTIVE','DELINQUENT','CLOSED','DEFAULTED','REJECTED','RESTRUCTURED','WRITTEN_OFF','SETTLED') NOT NULL DEFAULT 'PENDING'");
            }
        }
    } catch (PDOException $e) {
    ...
    loanAddCol($db, 'loans', 'debit_account_id', "INT UNSIGNED DEFAULT NULL");
    loanAddCol($db, 'loans', 'debit_account_number', "VARCHAR(30) DEFAULT NULL");
    loanAddCol($db, 'loans', 'guarantor_customer_id', "INT UNSIGNED DEFAULT NULL");
    loanAddCol($db, 'loans', 'guarantor_account_id', "INT UNSIGNED DEFAULT NULL");
    ...
    loanAddCol($db, 'loans', 'created_by', "INT UNSIGNED DEFAULT NULL");
    ...
    try {
        $schCol = $db->query("SHOW COLUMNS FROM loan_schedule LIKE 'status'")->fetch();
        if ($schCol && str_contains($schCol['Type'], 'enum(')) {
            if (!str_contains($schCol['Type'], 'UPCOMING')) {
                $db->exec("ALTER TABLE loan_schedule MODIFY COLUMN status ENUM('UPCOMING','DUE','PAID','MISSED','PARTIALLY_PAID','WAIVED') NOT NULL DEFAULT 'UPCOMING'");
            }
        }
    } catch (PDOException $e) {
...
function loanEnsureLoanFundReportingSchema(PDO $db): void {
    ...
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS loan_fund_transactions (
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            INDEX idx_lf_account (loan_fund_account_id),
            INDEX idx_lf_ref (transaction_ref),
            INDEX idx_lf_branch (branch)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    } catch (PDOException $e) {
    ...
    case 'GET':
        if ($id !== null) {
            ...
            try {
                ...
                $stmt = $db->prepare('SELECT * FROM loans ' . $where . ' ORDER BY created_at DESC LIMIT :limit OFFSET :offset');
                ...
                try {
                    ...
                    if (!empty($txns)) {
                        ...
                        $revStmt = $db->prepare(
                            "UPDATE transactions
                                SET status = 'REVERSED',
                                    memo = CONCAT(IFNULL(memo,''), :memo_suffix)
                        ...
                        if ($fundAccountId > 0) {
                            ...
                            $insGlStmt = $db->prepare(
                                "INSERT INTO general_ledger
                                 ...
                                 VALUES
                                    (:code, :name, :debit, :credit, CURDATE(), :ref, :desc, :branch, :posted_by, :txn_type, :contra)"
                            );
                            $insFundTxnStmt = $db->prepare(
                                "INSERT INTO loan_fund_transactions
                                 ...
                                 VALUES
                                    (:ref, :faid, :lid, :txn_ref, CURDATE(), 'CREDIT', :desc, :amount, :bal_after, :branch, :operator)"
                            );
            ...
            try {
                ...
                try {
                    $db->exec("CREATE TABLE IF NOT EXISTS loan_auto_pay_log (
                        ...
                        INDEX idx_apay_loan (loan_id),
                        INDEX idx_apay_date (processed_at)
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
                } catch (PDOException $e) {
                ...
                foreach ($loans as $loan) {
                    ...
                    foreach ($dueInstallments as &$inst) {
                        ...
                        foreach ($paymentAccounts as $pAcct) {
                            ...
                            try {
                                $db->prepare(
                                    "INSERT INTO transactions (ref, type, direction, account, branch, amount, status, category, module, description, memo, created_at)
                                     VALUES (:ref, 'LOAN_REPAYMENT', 'debit', :acct, :branch, :amt, 'POSTED', 'Loan Repayment', 'Loans', :desc, :memo, NOW())"
                                )->execute([
                                    ':ref'   => $txnRef,
                                    ':acct'  => $pAcct['account_number'],
                                    ':branch'=> $loan['branch'] ?? '',
                                    ':amt'   => $pAcct['amount'],
                                    ':desc'  => "Auto repayment — $loanNumber Installment #$instNumber",
                                ...
                                error_log('[AutoPay] Transaction insert failed for ' . $txnRef . ': ' . $e->getMessage());
                            }

                            // Log to auto-pay table
                            try {
                                // ★ GL INTEGRATION (AUTO-PAY)
                                // Principal: DR 2000 (Customer Deposits) / CR 1201 (Loans Receivable)
                                ...
                                // Interest: DR 2000 (Customer Deposits) / CR 4200 (Loan Interest Income)
                                if ($pAcct['interest'] > 0) {
                                    ...
                                    if ($liId) {
                                        $balAfter = loanGetGLIncomeBalance($db, '4200', (string)($loan['branch'] ?? ''));
                                ...
                                error_log('[AutoPay] Log insert failed: ' . $e->getMessage());
                            }
                        }
        ...
        */
        ...
        try {
            ...
            try {
                ...
                $maxNum = $db->query(
                    "SELECT MAX(CAST(SUBSTRING(loan_number, " . strlen($prefix) . ") AS UNSIGNED)) AS max_num FROM loans WHERE loan_number LIKE '" . $prefix . "%' AND loan_number REGEXP '^LN-" . $year . "-[0-9]{3}$'"
                )->fetch();
            } catch (PDOException $e) {
