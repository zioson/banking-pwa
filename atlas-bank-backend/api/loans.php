...
function loanEnsureTransactionsCompat(PDO $db): void {
 ...
 'amount' => "DECIMAL(20,2) NOT NULL DEFAULT 0",
 'status' => "VARCHAR(30) NOT NULL DEFAULT 'PENDING'",
 'category' => "VARCHAR(100); } catch (Exception $_) {}
 }
}
...
function loanInsertTransaction(PDO $db, array $row): void {
 $cols = [];
 try {
 $colRows = $db->query("SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'transactions'")->fetchAll(PDO::FETCH_COLUMN, 0);
...
function loanEnsureSchema(PDO $db): void {
 ...
 try {
 $col = $db->query("SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'loan_schedule' AND column_name = 'due'")->fetch();
 if ($col) {
 ...
 loanAddCol($db, 'loan_schedule', 'settled_via', "VARCHAR(30);
 ...
 loanAddCol($db, 'loans', 'product_type', "VARCHAR(50);
 ...
 loanAddCol($db, 'loans', 'debit_account_id', "INT DEFAULT NULL");
 loanAddCol($db, 'loans', 'debit_account_number', "VARCHAR(30);
 loanAddCol($db, 'loans', 'guarantor_customer_id', "INT DEFAULT NULL");
 loanAddCol($db, 'loans', 'guarantor_account_id', "INT DEFAULT NULL");
 loanAddCol($db, 'loans', 'guarantor_account_number', "VARCHAR(30);
 ...
 loanAddCol($db, 'loans', 'created_by', "INT DEFAULT NULL");
 loanAddCol($db, 'loans', 'loan_module', "VARCHAR(20);
 loanAddCol($db, 'loans', 'insurance_fee', "DECIMAL(18,2);
...
switch ($method) {
 ...
 "UPDATE transactions
 SET status = 'REVERSED',
 memo = CONCAT(COALESCE(memo,''), :memo_suffix)
 ...
 Processes all ACTIVE/DELINQUENT loans with auto_deduct = TRUE whose installments are due.
 ...
 // Find all eligible loans: ACTIVE or DELINQUENT with auto_deduct = TRUE
 $loanStmt = $db->prepare(
 "SELECT * FROM loans WHERE status IN ('ACTIVE','DELINQUENT') AND auto_deduct = TRUE ORDER BY id ASC"
 );
 $loanStmt->execute();
 ...
 */
 ...
 $prefix = 'LN-' . $year . '-';
 try {
 // ~ filter excludes corrupted rows like 'LN-2026-9.2233720368548E+18-d1'
