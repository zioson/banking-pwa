...
$db->exec("CREATE TABLE IF NOT EXISTS transaction_deductions (
    ...
    amount          DECIMAL(20,2) NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    INDEX idx_td_txn (transaction_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
...
try {
    $db->exec("CREATE TABLE IF NOT EXISTS profit_ledger (
        ...
        INDEX idx_pl_source_ref (source_ref),
    ...
    case 'GET':
        if ($id !== null) {
            ...
            try {
                ...
                $stmt = $db->prepare(
                    'SELECT * FROM transactions ' . $where . ' ORDER BY `' . $sortBy . '` ' . $sortOrder . ' LIMIT :limit OFFSET :offset'
                );
        ...
        try {
            ...
            try {
                foreach ([
                    ...
                    'parent_id'     => 'INT UNSIGNED DEFAULT NULL'  // ★ FIX (TXN-002)
                ] as $colName => $colDef) {
                    $col = $db->query("SHOW COLUMNS FROM transactions LIKE '$colName'")->fetch();
            ...
            try {
                ...
                error_log('[Transactions POST] SHOW COLUMNS failed: ' . $e->getMessage());
            }
                ...
                try {
                    ...
                    if ($idempotencyKey) {
                        try {
                            ...
                            $stmtIdem = $dbIdem->prepare("INSERT INTO idempotency_keys (`key`, operator_id, response_json, expires_at) VALUES (:key, :op_id, :resp, DATE_ADD(NOW(), INTERVAL 1 HOUR)) ON DUPLICATE KEY UPDATE response_json = VALUES(response_json)");
