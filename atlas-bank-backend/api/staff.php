...
try {
 ...
 if (empty($cols)) {
 $db->exec("ALTER TABLE staff ADD COLUMN force_password_change SMALLINT NOT NULL DEFAULT 0 AFTER locked_until");
 }
 ...
 if (empty($cols3)) {
 $db->exec("ALTER TABLE staff ADD COLUMN timezone VARCHAR(50);
 }
 ...
 if (empty($cols4)) {
 $db->exec("ALTER TABLE staff ADD COLUMN locale VARCHAR(10);
 }
 ...
 if (empty($cols5)) {
 $db->exec("ALTER TABLE staff ADD COLUMN notifications_enabled SMALLINT NOT NULL DEFAULT 1 AFTER locale");
 }
 ...
 if (empty($cols7)) {
 $db->exec("ALTER TABLE staff ADD COLUMN deactivation_reason VARCHAR(500);
 }
 ...
 if (empty($cols8)) {
 $db->exec("ALTER TABLE staff ADD COLUMN suspension_reason VARCHAR(500);
 }
...
function attachAssignments(PDO $db, array &$record): void {
 ...
 $cols = [];
 foreach ($db->query("SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'staff_modules'")->fetchAll(PDO::FETCH_ASSOC) as $_c) {
 $cols[strtolower($_c['Field'])] = true;
 }
 if (!isset($cols['access_level'])) {
 $db->exec("ALTER TABLE staff_modules ADD COLUMN access_level VARCHAR(30) NOT NULL DEFAULT 'FULL' AFTER module_name");
 }
