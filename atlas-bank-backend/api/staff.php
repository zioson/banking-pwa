...
$db->exec("CREATE TABLE IF NOT EXISTS staff (
    ...
    employment_status     VARCHAR(20)  NOT NULL DEFAULT 'ACTIVE' COMMENT 'ACTIVE, INACTIVE, SUSPENDED',
...
try {
    $cols = $db->query("SHOW COLUMNS FROM staff LIKE 'force_password_change'")->fetchAll();
    if (empty($cols)) {
        $db->exec("ALTER TABLE staff ADD COLUMN force_password_change TINYINT(1) NOT NULL DEFAULT 0 AFTER locked_until");
    }
    $cols2 = $db->query("SHOW COLUMNS FROM staff LIKE 'password_changed_at'")->fetchAll();
    if (empty($cols2)) {
        $db->exec("ALTER TABLE staff ADD COLUMN password_changed_at TIMESTAMP NULL DEFAULT NULL AFTER force_password_change");
    }
    $cols3 = $db->query("SHOW COLUMNS FROM staff LIKE 'timezone'")->fetchAll();
    if (empty($cols3)) {
        $db->exec("ALTER TABLE staff ADD COLUMN timezone VARCHAR(50) DEFAULT 'Africa/Douala' AFTER password_changed_at");
    }
    $cols4 = $db->query("SHOW COLUMNS FROM staff LIKE 'locale'")->fetchAll();
    if (empty($cols4)) {
        $db->exec("ALTER TABLE staff ADD COLUMN locale VARCHAR(10) DEFAULT 'en' AFTER timezone");
    }
    $cols5 = $db->query("SHOW COLUMNS FROM staff LIKE 'notifications_enabled'")->fetchAll();
    if (empty($cols5)) {
        $db->exec("ALTER TABLE staff ADD COLUMN notifications_enabled TINYINT(1) NOT NULL DEFAULT 1 AFTER locale");
    }
    $cols6 = $db->query("SHOW COLUMNS FROM staff LIKE 'profile_picture'")->fetchAll();
    if (empty($cols6)) {
    ...
    $cols7 = $db->query("SHOW COLUMNS FROM staff LIKE 'deactivation_reason'")->fetchAll();
    if (empty($cols7)) {
        $db->exec("ALTER TABLE staff ADD COLUMN deactivation_reason VARCHAR(500) DEFAULT NULL AFTER profile_picture");
    }
    $cols8 = $db->query("SHOW COLUMNS FROM staff LIKE 'suspension_reason'")->fetchAll();
    if (empty($cols8)) {
        $db->exec("ALTER TABLE staff ADD COLUMN suspension_reason VARCHAR(500) DEFAULT NULL AFTER deactivation_reason");
    }
...
$db->exec("CREATE TABLE IF NOT EXISTS staff_branches (
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_staff_branch (staff_id, branch_name),
...
$db->exec("CREATE TABLE IF NOT EXISTS staff_modules (
    ...
    access_level ENUM('FULL','VIEW_ONLY') NOT NULL DEFAULT 'FULL',
    PRIMARY KEY (id),
    UNIQUE KEY uk_staff_module (staff_id, module_name),
    ...
    foreach ($rawModules as $m) {
        ...
        if ($lower === 'all' || $lower === 'all modules') {
            $allModules = ['Dashboard','Customers','Accounts','Transactions','Loans','Approvals',
                'Expenses','Operating Fund','GL Accounts','Internal Audit','Branch Management',
...
function attachAssignments(PDO $db, array &$record): void {
    ...
    try {
        ...
        $cols = [];
        foreach ($db->query("SHOW COLUMNS FROM staff_modules")->fetchAll(PDO::FETCH_ASSOC) as $_c) {
            $cols[strtolower($_c['Field'])] = true;
        }
        if (!isset($cols['access_level'])) {
            $db->exec("ALTER TABLE staff_modules ADD COLUMN access_level ENUM('FULL','VIEW_ONLY') NOT NULL DEFAULT 'FULL' AFTER module_name");
        }
...
switch ($method) {
    ...
    case 'GET':
            ...
            try {
                ...
                $stmt = $db->prepare(
                    "SELECT id, uuid, actor, action, entity, entity_id, result, ip, details, module, category, user_agent,
                     ...
                     ORDER BY COALESCE(timestamp, created_at) DESC LIMIT :lim OFFSET :off"
                );
        ...
        if (!empty($items)) {
            ...
            try {
                ...
                // after a detail API call converts cached modules from strings to objects.
    ...
    case 'POST':
        ...
        // User MUST change on first login (force_password_change = 1).
    ...
    case 'PUT':
        ...
        if ($isOwnSelfRoute && $subRes !== null) {
            ...
            if ($subRes === 'password') {
                ...
                $db->prepare("UPDATE staff SET password_hash = ?, failed_login_attempts = 0, account_locked = 0, locked_until = NULL, force_password_change = 0, password_changed_at = NOW(), updated_at = NOW() WHERE id = ?")
        ...
        if ($subRes === 'password') {
            ...
            // to change it again. force_password_change=1 is only for NEW accounts
            ...
            $db->prepare("UPDATE staff SET password_hash = ?, failed_login_attempts = 0, account_locked = 0, locked_until = NULL, force_password_change = ?, password_changed_at = NOW(), updated_at = NOW() WHERE id = ?")
