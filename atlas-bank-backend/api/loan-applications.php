...
function laEnsureSchema(PDO $db): void {
    ...
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS loan_applications (
            ...
            status ENUM('PENDING','UNDER_REVIEW','APPROVED','REJECTED','WITHDRAWN','DISBURSED') DEFAULT 'PENDING',
            branch VARCHAR(200) DEFAULT '',
            applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            decided_by INT DEFAULT NULL,
            decided_at TIMESTAMP NULL,
            decision_reason TEXT DEFAULT NULL,
            loan_id INT DEFAULT NULL COMMENT 'ID of the loan created from this application',
            guarantor_customer_id INT DEFAULT NULL COMMENT 'Optional guarantor customer linked to this application',
            guarantor_account_id INT DEFAULT NULL COMMENT 'Optional guarantor repayment fallback account',
            guarantor_account_number VARCHAR(30) DEFAULT '' COMMENT 'Cached guarantor account number',
            INDEX idx_ref (ref),
    ...
    laAddCol($db, 'loan_applications', 'auto_deduct', "TINYINT(1) DEFAULT 1");
    laAddCol($db, 'loan_applications', 'interest_included', "TINYINT(1) DEFAULT 1");
    ...
    laAddCol($db, 'loan_applications', 'requires_double_approval', "TINYINT(1) DEFAULT 0");
    ...
    laAddCol($db, 'loan_applications', 'disbursement_account_id', "INT DEFAULT NULL COMMENT 'Customer account selected for loan disbursement destination'");
    laAddCol($db, 'loan_applications', 'guarantor_customer_id', "INT DEFAULT NULL COMMENT 'Optional guarantor customer linked to this application'");
    laAddCol($db, 'loan_applications', 'guarantor_account_id', "INT DEFAULT NULL COMMENT 'Optional guarantor repayment fallback account'");
    laAddCol($db, 'loan_applications', 'guarantor_account_number', "VARCHAR(30) DEFAULT '' COMMENT 'Cached guarantor account number'");
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM loan_applications LIKE 'status'")->fetch();
        if ($col && str_contains($col['Type'], 'enum(')) {
            if (!str_contains($col['Type'], 'DISBURSED')) {
                $db->exec("ALTER TABLE loan_applications MODIFY COLUMN status ENUM('PENDING','UNDER_REVIEW','APPROVED','REJECTED','WITHDRAWN','DISBURSED') NOT NULL DEFAULT 'PENDING'");
            }
        }
    } catch (PDOException $e) {
    ...
    laAddCol($db, 'loan_applications', 'loan_id', "INT DEFAULT NULL COMMENT 'ID of the loan created from this application (NULL if not yet disbursed)'");
}
...
function laEnsureApprovalsSchema(PDO $db): void {
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM approvals LIKE 'details'")->fetch();
        if (!$col) {
            $db->exec("ALTER TABLE approvals ADD COLUMN details TEXT DEFAULT NULL AFTER value");
        }
    } catch (PDOException $e) {
...
switch ($method) {
    ...
    case 'GET':
        try {
            ...
            if ($id !== null) {
                ...
                $stmt = $db->prepare(
                    'SELECT * FROM loan_applications ' . $where . ' ORDER BY applied_at DESC LIMIT :limit OFFSET :offset'
                );
    ...
    case 'POST':
        ...
        try {
            ...
            try {
                $db->exec("CREATE TABLE IF NOT EXISTS loans (id INT AUTO_INCREMENT PRIMARY KEY) ENGINE=InnoDB");
                $eligStmt = $db->prepare(
                    "SELECT id, loan_number, status, principal FROM loans
                     WHERE customer_id = :cid
                       AND status IN ('ACTIVE', 'DELINQUENT', 'WRITTEN_OFF', 'DEFAULTED')
                     ORDER BY FIELD(status, 'ACTIVE', 'DELINQUENT', 'WRITTEN_OFF', 'DEFAULTED')
                     LIMIT 1"
                );
            ...
            try {
                $maxNum = $db->query(
                    "SELECT MAX(CAST(SUBSTRING(ref, " . strlen($prefix) . ") AS UNSIGNED)) AS max_num
                     FROM loan_applications WHERE ref LIKE '" . $prefix . "%'
                     AND ref REGEXP '^LA-" . $year . "-[0-9]{3,4}$'"
                )->fetch();
            } catch (PDOException $e) {
                $maxNum = ['max_num' => null];
            }
