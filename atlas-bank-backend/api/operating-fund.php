...
function ensureGeneralLedgerTable(PDO $db): void {
    $check = $db->query("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'general_ledger'")->fetch();
    if (!$check) {
...
function ensureChartOfAccounts(PDO $db): void {
    ...
    $tableCheck = $db->query("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'chart_of_accounts'")->fetch();
    if (!$tableCheck) {
    ...
    $seedStmt = $db->prepare(
        "INSERT INTO chart_of_accounts (account_code, account_name, account_type, description, is_active)
         VALUES (:code, :name, :type, :desc, TRUE)
...
function generateOpRef(PDO $db, string $type): string {
    ...
    // ★ FIX (OP-FUND-010): Removed COUNT(*) + CURRENT_DATE approach.
...
function buildGLInsertSQL(): string {
    return "INSERT INTO general_ledger
        ...
        VALUES (CURRENT_DATE, :ref, :acode, :aname, :debit, :credit,
    ...
    case 'GET':
        try {
            ...
            if ($subResource === 'transactions') {
                ...
                $stmt = $db->prepare("SELECT * FROM operating_account_transactions $where ORDER BY date DESC, id DESC LIMIT CAST(:limit AS INTEGER) OFFSET CAST(:offset AS INTEGER)");
    ...
    case 'POST':
        ...
        if ($action === 'credit') {
            ...
            try {
                ...
                $ins = $db->prepare("INSERT INTO operating_account_transactions
                    ...
                    VALUES (:ref, :aid, CURRENT_DATE, 'CREDIT', :desc, :amt, :bal, :op, :stype, :sref, :cat, :gl, :branch)");
                $ins->execute([
        ...
        // ─────────────────────────────────────────────
            ...
            try {
                ...
                $ins = $db->prepare("INSERT INTO operating_account_transactions
                    (ref, operating_account_id, date, type, description, amount, balance_after, operator, source_type, source_ref, category, gl_code, branch)
                    VALUES (:ref, :aid, CURRENT_DATE, 'DEBIT', :desc, :amt, :bal, :op, :stype, :sref, :cat, :gl, :branch)");
                $ins->execute([
                    ':ref' => $ref, ':aid' => $account['id'],
                    ':desc' => $desc,
