...
function ensureOpFundColumns(PDO $db): void {
 ...
 if (!$check) {
 $type = ($col === 'description') ? 'TEXT' : "VARCHAR(100);
...
function ensureGeneralLedgerTable(PDO $db): void {
 $check = $db->query("SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'general_ledger'")->fetch();
 if (!$check) {
...
function ensureChartOfAccounts(PDO $db): void {
 ...
 $tableCheck = $db->query("SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'chart_of_accounts'")->fetch();
 if (!$tableCheck) {
