...
try {
    $db->exec('ALTER TABLE "policies" RENAME COLUMN "effective_date" TO "effective_from"');
} catch (PDOException $e) { /* column doesn't exist or already renamed */ }

// Ensure effective_to column exists and has proper default
try {
    $db->exec('ALTER TABLE "policies" ALTER COLUMN "effective_to" TYPE DATE');
} catch (PDOException $e) { /* ignore */ }

// Ensure status column has proper default
try {
    $db->exec('ALTER TABLE "policies" ALTER COLUMN "status" TYPE VARCHAR(20)');
} catch (PDOException $e) { /* ignore */ }
...
switch ($method) {
    ...
    case 'POST':
        ...
        if ($eto && $efrom && $eto < $efrom) {
            validationError(['effective_to' => 'Effective To date must be From date.']);
        }
