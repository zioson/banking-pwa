...
$_cols = [];
foreach ($db->query("SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'branches'")->fetchAll(PDO::FETCH_ASSOC) as $col) {
 $_cols[strtolower($col['Field'])] = true;
}
if (!isset($_cols['address'])) $db->exec("ALTER TABLE branches ADD COLUMN address VARCHAR(500);
if (!isset($_cols['phone'])) $db->exec("ALTER TABLE branches ADD COLUMN phone VARCHAR(50);
if (!isset($_cols['manager'])) $db->exec("ALTER TABLE branches ADD COLUMN manager VARCHAR(255);
