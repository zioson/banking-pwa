...
$db->exec("CREATE TABLE IF NOT EXISTS branches (
    ...
    code        VARCHAR(20)  NOT NULL COMMENT 'Unique branch code, e.g. HQ, BAF, DOU',
    name        VARCHAR(255) NOT NULL,
    region      VARCHAR(100) DEFAULT NULL,
    country     VARCHAR(100) NOT NULL DEFAULT 'CM',
    status      VARCHAR(20)  NOT NULL DEFAULT 'ACTIVE' COMMENT 'ACTIVE or INACTIVE',
    address     VARCHAR(500) DEFAULT NULL COMMENT 'Physical branch address',
    phone       VARCHAR(50)  DEFAULT NULL COMMENT 'Branch contact phone',
    manager     VARCHAR(255) DEFAULT NULL COMMENT 'Branch manager name',
    opened_date DATE         DEFAULT NULL COMMENT 'Branch opening date',
    created_at  TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    updated_at  TIMESTAMP    DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    UNIQUE KEY uk_branches_code (code),
    INDEX idx_branches_status (status),
    INDEX idx_branches_region (region)
...
if (!isset($_cols['address']))      $db->exec("ALTER TABLE branches ADD COLUMN address VARCHAR(500) DEFAULT NULL AFTER country");
if (!isset($_cols['phone']))       $db->exec("ALTER TABLE branches ADD COLUMN phone VARCHAR(50) DEFAULT NULL AFTER address");
if (!isset($_cols['manager']))     $db->exec("ALTER TABLE branches ADD COLUMN manager VARCHAR(255) DEFAULT NULL AFTER phone");
if (!isset($_cols['opened_date'])) $db->exec("ALTER TABLE branches ADD COLUMN opened_date DATE DEFAULT NULL AFTER manager");
