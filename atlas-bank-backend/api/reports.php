...
function reportSafeAddCol(PDO $db, string $table, string $col, string $def): void {
 try {
 $r = $db->query("SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = '$table' AND column_name = '$col'")->fetch();
...
function reportAddCol(PDO $db, string $col, string $def): void {
 try {
 $r = $db->query("SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'profit_ledger' AND column_name = '$col'")->fetch();
...
function ensureProfitLedgerSchema(PDO $db): void {
 ...
 reportAddCol($db, 'gl_code', "VARCHAR(20);
 reportAddCol($db, 'gl_account_name', "VARCHAR(100);
 reportAddCol($db, 'gl_type', "VARCHAR(20);
 reportAddCol($db, 'category', "VARCHAR(50);
 reportAddCol($db, 'source_ref', "VARCHAR(50);
 reportAddCol($db, 'source_type', "VARCHAR(50);
 reportAddCol($db, 'account_number', "VARCHAR(50);
 reportAddCol($db, 'account_type', "VARCHAR(50);
 reportAddCol($db, 'customer_name', "VARCHAR(200);
 reportAddCol($db, 'branch', "VARCHAR(100);
 reportAddCol($db, 'gross_amount', "DECIMAL(18,2);
 reportAddCol($db, 'fee_amount', "DECIMAL(18,2);
 reportAddCol($db, 'fee_pct', "DECIMAL(8,4);
 reportAddCol($db, 'fee_mode', "VARCHAR(50);
 reportAddCol($db, 'operator', "VARCHAR(100);
 reportAddCol($db, 'description', "TEXT");
 reportAddCol($db, 'gl_category', "VARCHAR(100);
 reportAddCol($db, 'total_debit', "DECIMAL(20,2);
 reportAddCol($db, 'total_credit', "DECIMAL(20,2);
 reportAddCol($db, 'net_amount', "DECIMAL(20,2);
...
function ensureBalanceTrendsSchema(PDO $db): void {
 ...
 reportSafeAddCol($db, 'balance_trends', 'total_available', "DECIMAL(20,2);
}
...
function ensureTransactionsSchema(PDO $db): void {
 reportSafeAddCol($db, 'transactions', 'net_amount', "DECIMAL(20,2);
 reportSafeAddCol($db, 'transactions', 'fee_mode', "VARCHAR(50);
 reportSafeAddCol($db, 'transactions', 'total_tax', "DECIMAL(20,2);
...
function reportHasTable(PDO $db, string $table): bool {
 try {
 $stmt = $db->prepare("SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = ?");
 $stmt->execute([$table]);
...
function ensureLoanFundTxSchema(PDO $db): void {
 ...
 fund_type VARCHAR(30) NOT NULL DEFAULT 'LOAN_FUND',
 ...
 reportSafeAddCol($db, 'loan_fund_transactions', 'branch', "VARCHAR(20);
 reportSafeAddCol($db, 'loan_fund_transactions', 'operator', "VARCHAR(200);
 reportSafeAddCol($db, 'loan_fund_transactions', 'transaction_ref', "VARCHAR(50);
 reportSafeAddCol($db, 'loan_fund_transactions', 'ref', "VARCHAR(50);
 }
 try {
 $db->exec("
...
switch ($method) {
 ...
 try {
 $s = $db->prepare("SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE status='APPROVED' AND MONTH(date) = MONTH(CURRENT_DATE) AND YEAR(date) = YEAR(CURRENT_DATE)" . $dashBrFilter);
 rptBindBranchFilter($db, $s, $staff, $branch); $s->execute();
 ...
 // FIX: Wrap in try-catch — table may still be missing columns after migration
 $recentTransactions = [];
 try {
 ...
 $start = new TIMESTAMPTZ($dateFrom);
 $end = new TIMESTAMPTZ($dateTo);
 $interval = $start->diff($end);
 $prevEnd = (new TIMESTAMPTZ($dateFrom))->modify('-1 day');
 ...
 FROM profit_ledger";
 $liSelect = "SELECT
 CONCAT('LI-', lft.id) AS id,
 '4200' AS gl_code,
 'Loan Interest Income' AS gl_account_name,
 'INCOME' AS gl_type,
 'LOAN_INTEREST' AS category,
 COALESCE(NULLIF(lft.transaction_ref,''), NULLIF(lft.ref,''), CONCAT('LFT-', lft.id)) AS source_ref,
 ...
 $dateExpr = "TO_CHAR(created_at - (EXTRACT(ISODOW FROM created_at)::int - 1) * INTERVAL '1 day', 'YYYY-MM-DD')";
 $expDateExpr = "TO_CHAR(\"date\" - (EXTRACT(ISODOW FROM \"date\")::int - 1) * INTERVAL '1 day', 'YYYY-MM-DD')";
 } else {
 ...
 : ($trendMode === 'weekly'
 ? "TO_CHAR(\"date\" - (EXTRACT(ISODOW FROM \"date\")::int - 1) * INTERVAL '1 day', 'YYYY-MM-DD')"
 : "date");
 ...
 $db->exec("ALTER TABLE transactions ADD COLUMN fee DECIMAL(20,2);
 }
 try { $db->query("SELECT fee_pct FROM transactions LIMIT 1"); } catch (PDOException $e) {
 $db->exec("ALTER TABLE transactions ADD COLUMN fee_pct DECIMAL(8,4);
 }
 ...
 $existingCols = $db->query("SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'profit_ledger'")->fetchAll(PDO::FETCH_COLUMN, 0);
