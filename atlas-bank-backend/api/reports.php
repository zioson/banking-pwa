...
function reportAddCol(PDO $db, string $col, string $def): void {
    try {
        $r = $db->query("SHOW COLUMNS FROM profit_ledger LIKE '$col'")->fetch();
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════════════
                ...
                if ($trendMode === 'monthly') {
                    ...
                    $expDateExpr = "DATE_FORMAT(date, '%Y-%m')";
                } elseif ($trendMode === 'weekly') {
                    $dateExpr = "DATE_FORMAT(DATE_SUB(created_at, INTERVAL WEEKDAY(created_at) DAY), '%Y-%m-%d')";
                    $expDateExpr = "DATE_FORMAT(DATE_SUB(date, INTERVAL WEEKDAY(date) DAY), '%Y-%m-%d')";
                } else {
                ...
                if ($includeLoanInterest) {
                    $liDateExpr = $trendMode === 'monthly'
                        ...
                        : ($trendMode === 'weekly'
                            ? "DATE_FORMAT(DATE_SUB(date, INTERVAL WEEKDAY(date) DAY), '%Y-%m-%d')"
                            : "date");
    ...
    case 'POST':
        ...
        try {
            ...
            if ($input['type'] === 'profit_loss') {
                ...
                $existingCols = $db->query("SHOW COLUMNS FROM profit_ledger")->fetchAll(PDO::FETCH_COLUMN, 0);
                ...
                try {
                    ...
                    try {
                        foreach (['fee' => 'DECIMAL(20,2) DEFAULT 0', 'fee_pct' => 'DECIMAL(8,4) DEFAULT 0'] as $col => $def) {
                            $c = $db->query("SHOW COLUMNS FROM transactions LIKE '$col'")->fetch();
                    ...
                    $feeColExists = (bool)$db->query("SHOW COLUMNS FROM transactions LIKE 'fee'")->fetch();
