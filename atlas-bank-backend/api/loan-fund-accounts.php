...
function lfEnsureSchema(PDO $db): void {
 ...
 lfAddCol($db, 'general_ledger', 'transaction_type', "VARCHAR(50);
 lfAddCol($db, 'general_ledger', 'contra_account', "VARCHAR(50);
 lfAddCol($db, 'general_ledger', 'branch', "VARCHAR(100);
 ...
 lfAddCol($db, 'loan_fund_transactions', 'branch', "VARCHAR(20);
...
switch ($method) {
 ...
 try {
 $opTable = $db->query("SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'operating_account_transactions'")->fetch(PDO::FETCH_NUM);
 if ($opTable) {
