...
function lfEnsureSchema(PDO $db): void {
    ...
    lfAddCol($db, 'general_ledger', 'transaction_type', "VARCHAR(50) DEFAULT '' AFTER posted_by");
    lfAddCol($db, 'general_ledger', 'contra_account', "VARCHAR(50) DEFAULT '' AFTER transaction_type");
    lfAddCol($db, 'general_ledger', 'branch', "VARCHAR(100) DEFAULT '' AFTER description");
    ...
    lfAddCol($db, 'loan_fund_transactions', 'branch', "VARCHAR(20) DEFAULT NULL AFTER balance_after");

    // Backfill branch for existing loan_fund_transactions where missing
    try {
        $db->exec(
            "UPDATE loan_fund_transactions t
             JOIN loans l ON l.id = t.loan_id
             SET t.branch = l.branch
             WHERE (t.branch IS NULL OR t.branch = '')
    ...
    try {
        $db->exec(
            "UPDATE general_ledger g
             JOIN loan_fund_transactions t ON t.ref = g.reference
             SET g.branch = t.branch
             WHERE (g.branch IS NULL OR g.branch = '')
                ...
                if ($type === 'DEBIT') {
                    ...
                    if ($isWriteoff) {
                        ...
                        try {
                            ...
                            // Reporting artifact
                            $db->prepare("INSERT INTO loan_fund_transactions (ref, loan_fund_account_id, loan_id, transaction_ref, date, type, description, amount, balance_after, branch, operator) VALUES (?, ?, ?, ?, CURDATE(), 'DEBIT', ?, ?, ?, ?, ?)")
                        ...
                        try {
                            ...
                            try {
                                $opTable = $db->query("SHOW TABLES LIKE 'operating_account_transactions'")->fetch(PDO::FETCH_NUM);
                                if ($opTable) {
                                    ...
                                    if ($opAcct && isset($opAcct['id'])) {
                                        $db->prepare(
                                            "INSERT INTO operating_account_transactions
                                             ...
                                             VALUES (?, ?, CURDATE(), 'DEBIT', ?, ?, ?, ?, '1200', 'LOAN_DISBURSEMENT_TRANSFER', ?)"
                                        )->execute([
                                            $ref,
                                            (int)$opAcct['id'],
                            ...
                            // Recompute GL 1200 after both entries above; this is the authoritative post-state.
                    ...
                    if ($isRepayment) {
                        ...
                        try {
                            ...
                            // Reporting artifact
                            $db->prepare("INSERT INTO loan_fund_transactions (ref, loan_fund_account_id, loan_id, transaction_ref, date, type, description, amount, balance_after, branch, operator) VALUES (?, ?, ?, ?, CURDATE(), 'CREDIT', ?, ?, ?, ?, ?)")
                              ->execute([$ref, $acctId, $loanId, $txnRef,
