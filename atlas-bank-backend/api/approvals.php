    ...
    case 'GET':
        try {
            ...
            $db->exec("CREATE TABLE IF NOT EXISTS approvals (
                ...
                entity_type   VARCHAR(50)     NOT NULL COMMENT 'TRANSACTION, LOAN, EXPENSE, SETTING, etc.',
                entity_id     INT UNSIGNED    DEFAULT NULL,
                scope_code    VARCHAR(50)     NOT NULL COMMENT 'Approval scope, e.g. TXN_REVERSE, LOAN_DISBURSE',
                status        ENUM('PENDING','APPROVED','REJECTED','CANCELLED') NOT NULL DEFAULT 'PENDING',
                submitted_by  INT UNSIGNED    DEFAULT NULL,
                branch        VARCHAR(20)     DEFAULT NULL,
                value         TEXT            DEFAULT NULL COMMENT 'Serialized value or summary',
                details       TEXT            DEFAULT NULL COMMENT 'JSON blob for extra context (e.g. reversal details)',
            ...
            if (!$col) {
                $db->exec("ALTER TABLE approvals ADD COLUMN details TEXT DEFAULT NULL COMMENT 'JSON blob for extra context' AFTER value");
            }

            // ── Auto-migrate: fix status ENUM to include CANCELLED ──
            try {
                $statusCol = $db->query("SHOW COLUMNS FROM approvals LIKE 'status'")->fetch();
                if ($statusCol && str_contains($statusCol['Type'], 'enum(') && !str_contains($statusCol['Type'], 'CANCELLED')) {
                    $db->exec("ALTER TABLE approvals MODIFY COLUMN status ENUM('PENDING','APPROVED','REJECTED','CANCELLED') NOT NULL DEFAULT 'PENDING'");
                }
            } catch (PDOException $e) {
            ...
            if ($id !== null) {
                ...
                $stmt = $db->prepare('SELECT a.*, s1.full_name AS submitted_by_name, s2.full_name AS decided_by_name FROM approvals a LEFT JOIN staff s1 ON a.submitted_by = s1.id LEFT JOIN staff s2 ON a.decided_by = s2.id ' . $where . ' ORDER BY a.submitted_at DESC LIMIT :limit OFFSET :offset');
