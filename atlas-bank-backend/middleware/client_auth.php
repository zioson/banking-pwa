...
function ensureClientPortalTables(PDO $db): void
{
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS customer_portal_users (
        ...
        CONSTRAINT uk_customer_portal_customer UNIQUE (customer_id),
        CONSTRAINT uk_customer_portal_username UNIQUE (username)
    )");
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_customer_portal_status ON customer_portal_users (status)"); } catch (PDOException $ie) {}
    // Trigger for updated_at auto-update (replaces MySQL ON UPDATE CURRENT_TIMESTAMP)
    try {
        $db->exec("CREATE OR REPLACE FUNCTION trigger_set_updated_at() RETURNS TRIGGER AS \$\$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; \$\$ LANGUAGE plpgsql");
        $db->exec("DROP TRIGGER IF EXISTS set_updated_at ON customer_portal_users");
        $db->exec("CREATE TRIGGER set_updated_at BEFORE UPDATE ON customer_portal_users FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at()");
    ...
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_customer_sessions_customer ON customer_sessions (customer_id)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_customer_sessions_exp ON customer_sessions (expires_at)"); } catch (PDOException $ie) {}
    ...
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_clh_username ON customer_login_history (username)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_clh_customer ON customer_login_history (customer_id)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_clh_created ON customer_login_history (created_at)"); } catch (PDOException $ie) {}
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS customer_trusted_devices (
        ...
        CONSTRAINT uk_ctd_customer_fingerprint UNIQUE (customer_id, device_fingerprint)
    )");
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_ctd_customer ON customer_trusted_devices (customer_id)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_ctd_exp ON customer_trusted_devices (expires_at)"); } catch (PDOException $ie) {}
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS customer_mfa_pending_tokens (
        ...
        expires_at TIMESTAMP NOT NULL,
        CONSTRAINT uk_cmp_token UNIQUE (token)
    )");
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_cmp_customer ON customer_mfa_pending_tokens (customer_id)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_cmp_exp ON customer_mfa_pending_tokens (expires_at)"); } catch (PDOException $ie) {}
    ...
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_cpc_customer ON customer_portal_consents (customer_id)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_cpc_accepted ON customer_portal_consents (accepted_at)"); } catch (PDOException $ie) {}
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS customer_statement_links (
        ...
        created_ip VARCHAR(50) DEFAULT NULL,
        CONSTRAINT uk_csl_token UNIQUE (token_hash)
    )");
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_csl_customer ON customer_statement_links (customer_id)"); } catch (PDOException $ie) {}
    try { $db->exec("CREATE INDEX IF NOT EXISTS idx_csl_exp ON customer_statement_links (expires_at)"); } catch (PDOException $ie) {}

    // Lightweight self-heal for environments with older table versions.
    try { $db->exec("ALTER TABLE customer_portal_users ADD COLUMN mfa_required BOOLEAN NOT NULL DEFAULT FALSE"); } catch (PDOException $e) {}
    try { $db->exec("ALTER TABLE customer_portal_users ADD COLUMN mfa_secret VARCHAR(255) DEFAULT NULL"); } catch (PDOException $e) {}
    try { $db->exec("ALTER TABLE customer_portal_users ADD COLUMN mfa_enrolled_at TIMESTAMP NULL"); } catch (PDOException $e) {}
...
function trustClientDevice(PDO $db, int $customerId, string $fingerprint, string $label = '', int $days = 90): void
{
    ...
    $stmt = $db->prepare(
        'INSERT INTO customer_trusted_devices (customer_id, device_fingerprint, label, trusted_at, last_seen, expires_at, is_active, created_ip)
         VALUES (:cid, :fp, :label, :trusted_at, :last_seen, :exp, TRUE, :ip)
         ON CONFLICT (customer_id, device_fingerprint) DO UPDATE SET label = EXCLUDED.label, trusted_at = EXCLUDED.trusted_at, last_seen = EXCLUDED.last_seen, expires_at = EXCLUDED.expires_at, is_active = TRUE, created_ip = EXCLUDED.created_ip'
    );
    $stmt->execute([
        ':cid' => $customerId,
        ':fp' => $fingerprint,
