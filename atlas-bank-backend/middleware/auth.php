...
function requireAuth(): array
{
 ...
 $now = new TIMESTAMPTZ();
 $expiresAt = new TIMESTAMPTZ($session['expires_at']);
 if ($now > $expiresAt) {
 ...
 if ($lastActivityStr) {
 $lastActivity = new TIMESTAMPTZ($lastActivityStr);
 ...
 if ($lockedUntil !== null) {
 $lockedAt = new TIMESTAMPTZ($lockedUntil);
 if ($now < $lockedAt) {
 ...
 error_log('[AUTH DEBUG] force_password_change = TRUE, staff/me/password detected. allowed=' . ($allowed ? 'true' : 'false') . ' method=' . $method . ' subRes=' . var_export($subRes, true));
 }
 ...
 $insStmt = $db->prepare('INSERT INTO staff_modules (staff_id, module_name) VALUES (:sid, :mod)');
